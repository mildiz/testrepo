...
def get_pie_chart(entered_site):
    ...
    filtered_df.loc[filtered_df['class'] == 0 , 'class'] = 2
...
def get_scatter_chart(value,entered_site):
    value = [float(x) for x in value]
    value0=value[0]
    value1=value[1]
    print(value,entered_site)

    filtered_df = spacex_df
    filtered_df.loc[filtered_df['class'] == 0 , 'class'] = 2
    filtered_df=filtered_df.query("`Payload Mass (kg)` >= @value0 and `Payload Mass (kg)` <= @value1")
